...
int8_t esp32_rmt2_bitstream_channel_id = RMT_LAST_TX_CHANNEL;
...
static mp_obj_t esp32_rmt_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args) {
    ...
    if (esp32_rmt2_bitstream_channel_id >= 0 && channel_id == esp32_rmt2_bitstream_channel_id) {
    ...
    check_esp_err(rmt2_driver_install_core1(config.channel));
...
static mp_obj_t esp32_rmt_bitstream_channel(size_t n_args, const mp_obj_t *args) {
    if (n_args > 0) {
        if (args[0] == mp_const_none) {
            esp32_rmt2_bitstream_channel_id = -1;
        } else {
            mp_int_t channel_id = mp_obj_get_int(args[0]);
            if (channel_id < 0 || channel_id > RMT_LAST_TX_CHANNEL) {
                mp_raise_ValueError(MP_ERROR_TEXT("invalid channel"));
            }
            esp32_rmt2_bitstream_channel_id = channel_id;
        }
    }
    if (esp32_rmt2_bitstream_channel_id < 0) {
        return mp_const_none;
    } else {
        return MP_OBJ_NEW_SMALL_INT(esp32_rmt2_bitstream_channel_id);
    }
}
