...
typedef struct _esp32_rmt2_obj_t {
    ...
    rmt_symbol_word_t symbols[SYMBOL_BUF_LEN];
    ...
    rmt_symbol_word_t recv_symbols[SYMBOL_BUF_LEN];
} esp32_rmt2_obj_t;
...
static bool IRAM_ATTR rmt_recv_done(rmt_channel_handle_t channel, const rmt_rx_done_event_data_t *edata, void *udata){
    ...
    if (!self->recv_locked) {
            ...
            memcpy(self->recv_symbols, edata->received_symbols, SYMBOL_BUF_LEN * sizeof(rmt_symbol_word_t));
    ...
    if (self->continue_rx) {
        rmt_receive(self->rx_channel, self->symbols, sizeof(self->symbols), &self->rx_config);
    }
...
static mp_obj_t esp32_rmt2_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args) {
    ...
    // FIXME configurable
    self->rx_ch_conf.mem_block_symbols = SYMBOL_BUF_LEN;
...
static void esp32_rmt2_print(const mp_print_t *print, mp_obj_t self_in, mp_print_kind_t kind) {
    esp32_rmt2_obj_t *self = MP_OBJ_TO_PTR(self_in);
    mp_printf(print, "RMT2 %u %s", self->recv_num_symbols, self->recv_locked ? "locked" : "unlocked");
}
...
static mp_obj_t esp32_rmt2_read_pulses(mp_obj_t self_in) {
    ...
    self->recv_locked = false;
    self->recv_num_symbols = 0;
    check_esp_err(rmt_receive(self->rx_channel, self->symbols, sizeof(self->symbols), &self->rx_config));
    return mp_const_none;
}
